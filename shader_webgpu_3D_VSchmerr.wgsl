...
struct SimFltValues {
    ...
    dz: f32,            // delta z
    dt: f32,            // delta t
    alpha: f32,         // coef. atenuacao
    f0: f32,      // frequencia central 
};
...
// (atenuacao alpha agora e' escalar uniforme em sim_flt_par.alpha)
...
fn velocity_kernel(@builtin(global_invocation_id) index: vec3<u32>) {
    ...
    let w0: f32 = 6.2831853071795864 * sim_flt_par.f0;
    let inv_w0sq: f32 = select(0.0, 1.0 / (w0 * w0), w0 > 0.0);
    ...
    if(x >= id_x_i && x < id_x_f && y >= id_y_i && y < id_y_f && z >= id_z_i && z < id_z_f) {
        ...
        if(rho > 0.0) {
            let cp_avg: f32 = 0.5 * (get_cp(x + 1, y, z) + get_cp(x, y, z));

            let eta_b: f32 = 2.0 * sim_flt_par.alpha * rho * cp_avg * cp_avg * cp_avg * inv_w0sq;

            let divv_xp: f32 = (get_vx(x + 1, y, z) - get_vx(x, y, z)) / dx
                             + (get_vy(x + 1, y, z) - get_vy(x + 1, y - 1, z)) / dy
                             + (get_vz(x + 1, y, z) - get_vz(x + 1, y, z - 1)) / dz;
            let divv_x0: f32 = (get_vx(x, y, z) - get_vx(x - 1, y, z)) / dx
                             + (get_vy(x, y, z) - get_vy(x, y - 1, z)) / dy
                             + (get_vz(x, y, z) - get_vz(x, y, z - 1)) / dz;
            let dDiv_dx: f32 = (divv_xp - divv_x0) / dx;
            
            let visc: f32 = eta_b * dDiv_dx;
            let vx: f32 = (vdsigmaxx_dx + vdsigmaxy_dy + vdsigmaxz_dz + visc) * dt / rho + get_vx(x, y, z);
            set_vx(x, y, z, vx);
        }
    }
    ...
    if(x >= id_x_i && x < id_x_f && y >= id_y_i && y < id_y_f && z >= id_z_i && z < id_z_f) {
        ...
        if(rho > 0.0) {
            let cp_avg: f32 = 0.5 * (get_cp(x, y + 1, z) + get_cp(x, y, z));
            let eta_b: f32 = 2.0 * sim_flt_par.alpha * rho * cp_avg * cp_avg * cp_avg * inv_w0sq;
            // d/dy (div v) no ponto da face de Vy (y+1/2):
            let divv_yp: f32 = (get_vx(x, y + 1, z) - get_vx(x - 1, y + 1, z)) / dx
                             + (get_vy(x, y + 1, z) - get_vy(x, y, z)) / dy
                             + (get_vz(x, y + 1, z) - get_vz(x, y + 1, z - 1)) / dz;
            let divv_y0: f32 = (get_vx(x, y, z) - get_vx(x - 1, y, z)) / dx
                             + (get_vy(x, y, z) - get_vy(x, y - 1, z)) / dy
                             + (get_vz(x, y, z) - get_vz(x, y, z - 1)) / dz;
            let dDiv_dy: f32 = (divv_yp - divv_y0) / dy;
            let visc: f32 = eta_b * dDiv_dy;
            let vy: f32 = (vdsigmaxy_dx + vdsigmayy_dy + vdsigmayz_dz + visc) * dt / rho + get_vy(x, y, z);
            set_vy(x, y, z, vy);
        }
    }
    ...
    if(x >= id_x_i && x < id_x_f && y >= id_y_i && y < id_y_f && z >= id_z_i && z < id_z_f) {
        ...
        let rho_z: f32 = 0.5 * (get_rho(x, y, z + 1) + get_rho(x, y, z));
        if(rho_z > 0.0) {
            let cp_z: f32 = 0.5 * (get_cp(x, y, z + 1) + get_cp(x, y, z));
            let eta_b: f32 = 2.0 * sim_flt_par.alpha * rho_z * cp_z * cp_z * cp_z * inv_w0sq;
            // d/dz (div v) no ponto da face de Vz (z+1/2):
            let divv_zp: f32 = (get_vx(x, y, z + 1) - get_vx(x - 1, y, z + 1)) / dx
                             + (get_vy(x, y, z + 1) - get_vy(x, y - 1, z + 1)) / dy
                             + (get_vz(x, y, z + 1) - get_vz(x, y, z)) / dz;
            let divv_z0: f32 = (get_vx(x, y, z) - get_vx(x - 1, y, z)) / dx
                             + (get_vy(x, y, z) - get_vy(x, y - 1, z)) / dy
                             + (get_vz(x, y, z) - get_vz(x, y, z - 1)) / dz;
            let dDiv_dz: f32 = (divv_zp - divv_z0) / dz;
            let visc: f32 = eta_b * dDiv_dz;
            let vz: f32 = (vdsigmaxz_dx + vdsigmayz_dy + vdsigmazz_dz + visc) * dt / rho_z + get_vz(x, y, z);
            set_vz(x, y, z, vz);
        }
    }
    ...
    if(idx_src_term != -1) {
        let rho_src: f32 = 0.5 * (get_rho(x, y, z + 1) + get_rho(x, y, z));
        if(rho_src > 0.0) {
            let val_src: f32 = get_source_term(sim_int_par.it, idx_src_term);
            let vz: f32 = get_vz(x, y, z) + val_src * dt / rho_src;
            set_vz(x, y, z, vz);
        }
    }

    let v_2_old: f32 = v_2;
    let v2: f32 = get_vx(x, y, z) * get_vx(x, y, z)
