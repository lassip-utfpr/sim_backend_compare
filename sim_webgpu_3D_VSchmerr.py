...
class SimulatorWebGPU(Simulator3D):
    def __init__(self, file_config):
        ...
        self._name = "WebGPU-Schmerr-viscous-3D"
    ...
    def implementation(self):
        ...
        with open('shader_webgpu_3D_VSchmerr.wgsl') as shader_file:
        ...
        if viscoelastic_attn:
            ...
            attn_freq_mhz = self._configs["simul_configs"]["attn_freq_mhz"]

            alpha_curve = np.load(self._configs["simul_configs"]["alpha_file"])   # Np/mm
            freq_band   = np.load(self._configs["simul_configs"]["freq_file"])    # MHz

            f_min, f_max = float(freq_band.min()), float(freq_band.max())
            if attn_freq_mhz < f_min or attn_freq_mhz > f_max:
                print(f"AVISO: attn_freq_mhz={attn_freq_mhz} fora da banda "
                      f"[{f_min:.3f}, {f_max:.3f}] MHz; alpha sera extrapolado pelas bordas.")

            alpha_np_mm = float(np.interp(attn_freq_mhz, freq_band, alpha_curve))
        else:
            alpha_np_mm = 0.0

        if self._show_debug:
            modo = "viscoelastica" if viscoelastic_attn else "elastica"
            print(f"Simulacao {modo}: alpha = {alpha_np_mm:.6f} Np/mm")

        # Arrays com parametros inteiros (i32) e ponto flutuante (f32) para rodar o simulador
        ord = self._coefs.shape[0]

        params_i32 = np.array([self._nx, self._ny, self._nz, self._n_steps, self._n_src, self._n_rec, self._n_pto_rec, ord, 0],
                               dtype=int32)
        params_f32 = np.array([self._roi.get_dx(), self._roi.get_dy(), self._roi.get_dz(), self._dt, alpha_np_mm, attn_freq_mhz], dtype=flt32)
